...
class MeetEvalCpwerAlignmentDriftSegmentReconciliationHandoffWriteOutputsTest(unittest.TestCase):
    def test_write_outputs_emits_handoff_and_receipt_artifacts(self) -> None:
        handoff_row = build_handoff_row(
            {
                ...
                "reconciliation_target": "results/figures/meeteval_cpwer_alignment_drift_segment_reconciliation_diagnostic.md",
                "expected_evidence": "results/tables/meeteval_cpwer_alignment_drift_segment_reconciliation_diagnostic_receipt.json",
            }
        )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            with patch(
                "src.meeteval_cpwer_alignment_drift_segment_reconciliation_handoff.PROJECT_ROOT",
                root,
            ):
            ...
            with outputs[0].open("r", encoding="utf-8-sig", newline="") as handle:
                ...
                self.assertEqual(list(reader)[0]["handoff_status"], "reconciliation_handoff_ready")
            self.assertIn("Reconciliation Handoff", outputs[2].read_text(encoding="utf-8"))
