...
class FrontierOperatorNextActionStatusHandoffOperatorBriefBridgeChecklistWriteOutputsTest(unittest.TestCase):
    def test_write_outputs_emits_csv_json_and_markdown(self) -> None:
        rows = build_bridge_checklist_rows(
            {
                ...
                "bridge_note": "Open the operator brief for meeteval_compatibility first.",
            }
        )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            with patch(
                "src.frontier_operator_next_action_status_handoff_operator_brief_bridge_checklist.PROJECT_ROOT",
                root,
            ):
