...
class MeetEvalCpwerAlignmentDriftSegmentTimingDiagnosticHandoffWriteOutputsTest(unittest.TestCase):
    def test_write_outputs_emits_handoff_and_receipt_artifacts(self) -> None:
        handoff_row = build_handoff_row(
            {"case_id": "HeavyOverlap", "mismatched_speaker_count": "1", "dominant_blocker": "SPEAKER_2 delta=0.5"}
        )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            with patch(
                "src.meeteval_cpwer_alignment_drift_segment_timing_diagnostic_handoff.PROJECT_ROOT",
                root,
            ):
            ...
            with outputs[0].open("r", encoding="utf-8-sig", newline="") as handle:
                ...
                self.assertEqual(list(reader)[0]["handoff_status"], "timing_handoff_ready")
            self.assertIn("Timing Diagnostic Handoff", outputs[2].read_text(encoding="utf-8"))
