...
class SpeakerProfileEmbeddingTrialExecutionReceiptRunbookBridgeChecklistWriteOutputsTest(unittest.TestCase):
    def test_write_outputs_emits_csv_json_and_markdown(self) -> None:
        rows = build_bridge_checklist_rows(
            {
                ...
                "runbook_note": "Confirm runbook before reopening readiness.",
            }
        )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            with patch(
                "src.speaker_profile_embedding_trial_execution_receipt_runbook_bridge_checklist.PROJECT_ROOT",
                root,
            ):
