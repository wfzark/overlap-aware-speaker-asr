...
class MeetEvalCpwerAlignmentDriftSegmentHandoffTest(unittest.TestCase):
    def test_build_handoff_row_targets_segment_inspection(self) -> None:
        row = build_handoff_row(
            {
                "case_id": "HeavyOverlap",
                "scaffold_status": "scaffold_only",
                "inspection_target": "Segment-level cleaned separated hypothesis export for HeavyOverlap.",
                "expected_evidence": "results/tables/meeteval_hypothesis_segments.jsonl",
            }
        )

        self.assertEqual(row["handoff_status"], "segment_handoff_ready")
