...
class MeetEvalCpwerAlignmentDriftSegmentSpeakerCountDiagnosticHandoffWriteOutputsTest(unittest.TestCase):
    def test_write_outputs_emits_handoff_and_receipt_artifacts(self) -> None:
        handoff_row = build_handoff_row(
            {"case_id": "HeavyOverlap", "mismatched_speaker_count": "1", "dominant_blocker": "SPEAKER_2 delta=1"}
        )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            with patch(
                "src.meeteval_cpwer_alignment_drift_segment_speaker_count_diagnostic_handoff.PROJECT_ROOT",
                root,
            ):
            ...
            self.assertIn("Speaker Count Diagnostic Handoff", outputs[2].read_text(encoding="utf-8"))
