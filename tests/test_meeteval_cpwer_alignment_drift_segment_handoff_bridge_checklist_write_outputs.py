...
class MeetEvalCpwerAlignmentDriftSegmentHandoffBridgeChecklistWriteOutputsTest(unittest.TestCase):
    def test_write_outputs_emits_csv_json_and_markdown(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            with patch(
                "src.meeteval_cpwer_alignment_drift_segment_handoff_bridge_checklist.PROJECT_ROOT",
                root,
            ):
            ...
            with csv_path.open("r", encoding="utf-8-sig", newline="") as handle:
                ...
                self.assertEqual(list(reader)[0]["handoff_status"], "segment_handoff_ready")
            self.assertIn(
