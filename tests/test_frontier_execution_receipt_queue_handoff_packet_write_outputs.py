...
from src.frontier_execution_receipt_queue_handoff_packet import (
    PACKET_COLUMNS,
    build_packet_rows,
    write_outputs,
)
...
class FrontierExecutionReceiptQueueHandoffPacketWriteOutputsTest(unittest.TestCase):
    def test_write_outputs_emits_csv_json_and_markdown(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            with patch(
                "src.frontier_execution_receipt_queue_handoff_packet.PROJECT_ROOT",
                root,
            ):
            ...
            with csv_path.open("r", encoding="utf-8-sig", newline="") as handle:
                ...
                self.assertEqual(parsed[0]["section_name"], "receipt_queue_status")
