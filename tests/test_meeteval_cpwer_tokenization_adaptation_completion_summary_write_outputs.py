...
class MeetEvalCpwerTokenizationAdaptationCompletionSummaryWriteOutputsTest(unittest.TestCase):
    def test_write_outputs_emits_completion_summary_artifacts(self) -> None:
        ...
        completion_row = build_completion_row(reconciliation_rows, diagnostic_rows)

        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            with patch("src.meeteval_cpwer_tokenization_adaptation_completion_summary.PROJECT_ROOT", root):
            ...
            self.assertIn("Tokenization Adaptation Completion Summary", md_path.read_text(encoding="utf-8"))
