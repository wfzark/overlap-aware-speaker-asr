...
from src.demo_walkthrough_review_pass_completion_summary import (
    COMPLETION_COLUMNS,
...
class DemoWalkthroughReviewPassCompletionSummaryWriteOutputsTest(unittest.TestCase):
    def test_write_outputs_emits_status_and_completion_summary_artifacts(self) -> None:
        ...
        status_row = build_status_row(steps, {"1"})
        completion_row = build_completion_summary_row(status_row)

        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            with patch("src.demo_walkthrough_review_pass_completion_summary.PROJECT_ROOT", root):
