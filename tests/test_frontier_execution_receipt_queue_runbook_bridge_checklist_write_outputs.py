...
from src.frontier_execution_receipt_queue_runbook_bridge_checklist import (
...
class FrontierExecutionReceiptQueueRunbookBridgeChecklistWriteOutputsTest(unittest.TestCase):
    def test_write_outputs_emits_csv_json_and_markdown(self) -> None:
        rows = build_bridge_checklist_rows(
            {
                ...
                "runbook_note": "Start with MeetEval as the current first receipt-queue target.",
            }
        )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            with patch(
                "src.frontier_execution_receipt_queue_runbook_bridge_checklist.PROJECT_ROOT",
                root,
            ):
