...
class FrontierOperatorNextActionStatusHandoffRunbookBridgeChecklistWriteOutputsTest(unittest.TestCase):
    def test_write_outputs_emits_csv_json_and_markdown(self) -> None:
        rows = build_bridge_checklist_rows(
            {
                "recommended_frontier": "meeteval_compatibility",
                "runbook_note": "Start with meeteval_compatibility as the current ready lane.",
            }
        )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            with patch(
                "src.frontier_operator_next_action_status_handoff_runbook_bridge_checklist.PROJECT_ROOT",
                root,
            ):
