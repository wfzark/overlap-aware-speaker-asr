...
class FrontierExecutionReceiptQueuePhaseCheckpointBridgeChecklistWriteOutputsTest(unittest.TestCase):
    def test_write_outputs_emits_csv_json_and_markdown(self) -> None:
        rows = build_bridge_checklist_rows(
            {
                "checkpoint_frontier": "meeteval_compatibility",
                "completion_signal": "receipt queue verification is complete",
            }
        )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            with patch(
                "src.frontier_execution_receipt_queue_phase_checkpoint_bridge_checklist.PROJECT_ROOT",
                root,
            ):
