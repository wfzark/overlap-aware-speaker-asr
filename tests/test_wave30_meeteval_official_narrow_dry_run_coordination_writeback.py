...
from src.wave30_meeteval_official_narrow_dry_run_coordination_writeback import (
...
class Wave30MeetevalOfficialNarrowDryRunCoordinationWritebackTest(unittest.TestCase):
    def test_build_coordination_rows_has_five_sections(self) -> None:
        self.assertEqual(len(build_coordination_rows()), 5)

    def test_build_fill_row_marks_wave30_official_coordination_complete(self) -> None:
    ...
    def test_run_coordination_writeback_requires_wave30_closure(self) -> None:
        with patch(
            "src.wave30_meeteval_official_narrow_dry_run_coordination_writeback.load_json_dict",
            side_effect=[
