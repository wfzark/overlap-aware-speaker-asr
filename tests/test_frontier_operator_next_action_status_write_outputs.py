...
from src.frontier_operator_next_action_status import (
    STATUS_COLUMNS,
    build_status_row,
    write_outputs,
)
...
class FrontierOperatorNextActionStatusWriteOutputsTest(unittest.TestCase):
    def test_write_outputs_emits_csv_json_and_markdown(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            with patch("src.frontier_operator_next_action_status.PROJECT_ROOT", root):
            ...
            self.assertIn("Next-Action Status", md_path.read_text(encoding="utf-8"))
