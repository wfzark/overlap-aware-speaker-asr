...
from src.meeteval_cpwer_alignment_drift_segment_handoff import (
    HANDOFF_COLUMNS,
...
class MeetEvalCpwerAlignmentDriftSegmentHandoffWriteOutputsTest(unittest.TestCase):
    def test_write_outputs_emits_handoff_and_receipt_artifacts(self) -> None:
        handoff_row = build_handoff_row(
            {
                ...
                "inspection_target": "results/figures/meeteval_cpwer_alignment_drift_segment_inspection.md",
                "expected_evidence": "results/tables/meeteval_cpwer_alignment_drift_segment_inspection_receipt.json",
            }
        )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            with patch("src.meeteval_cpwer_alignment_drift_segment_handoff.PROJECT_ROOT", root):
            ...
            self.assertIn("MeetEval cpWER Alignment Drift Segment Handoff", outputs[2].read_text(encoding="utf-8"))
