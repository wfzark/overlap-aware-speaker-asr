...
class MeetEvalCpwerAlignmentDriftSegmentScaffoldWriteOutputsTest(unittest.TestCase):
    def test_write_outputs_emits_scaffold_and_receipt_artifacts(self) -> None:
        scaffold_row = build_scaffold_row({"case_id": "HeavyOverlap"})
        receipt_rows = build_scaffold_receipt_rows(scaffold_row)

        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            with patch(
                "src.meeteval_cpwer_alignment_drift_segment_scaffold.PROJECT_ROOT",
                root,
            ):
            ...
            self.assertIn("Segment Scaffold", outputs[2].read_text(encoding="utf-8"))
