...
from .demo_wave100_presentation_writeback import build_extended_polish_rows as build_wave100_polish_rows
...
def assert_extension_preconditions(
    ...
    wave100_fill: dict[str, Any],
) -> None:
    if str(wave101_receipt.get("execution_status", "")) != "wave101_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave101 exploration baseline closure must be complete before wave101 demo extension"
        )
    if str(wave100_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave100 presentation writeback must be filled before wave101 extension")
    if str(wave100_fill.get("storyboard_receipt_status", "")) != "wave100_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave100 storyboard receipt must be wave100_presentation_extension_complete before wave101 extension"
        )


def build_wave101_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave101",
        "headline": "Wave101 exploration+baseline closure after Wave100 OppositeOverlap chain boundary",
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave100_polish_rows()
    if any(row["section_id"] == "frontier_wave101" for row in rows):
        return rows
    return rows + [build_wave101_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave101_presentation_extension_complete",
        "storyboard_scope": "wave101_storyboard_extension",
        "expected_inputs": "Wave101 exploration+baseline closure card.",
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave101_presentation_extension_complete",
        "walkthrough_scope": "wave101_walkthrough_extension",
        "expected_inputs": "Wave101 exploration+baseline closure card.",
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave101_presentation_polish_extension",
        "storyboard_receipt_status": "wave101_presentation_extension_complete",
        "walkthrough_receipt_status": "wave101_presentation_extension_complete",
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave101_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave101_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave101_presentation_writeback.md"
...
def run_wave101_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave101_receipt = load_json_dict("results/tables/wave101_exploration_baseline_closure_receipt.json")
    wave100_fill = load_json_dict("results/tables/demo_wave100_presentation_writeback.json")
    assert_extension_preconditions(wave101_receipt, wave100_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave101_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave101_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave101 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave101 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave101_presentation_writeback(force=args.force)
