        ...
        rows.append(
            {
                ...
                "prerequisite_artifact": "results/figures/frontier_execution_receipt_queue_handoff.md",
                "receipt_target": receipt_target,
                "checklist_goal": (
                    f"Verify the receipt-fill handoff for {frontier_name} before opening the execution receipt."
                ),
                "bridge_note": (
                    f"Handoff reports readiness_status={readiness_status} for {frontier_name}; "
...
def build_bridge_checklist_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        ...
        "",
        "This generated checklist turns the receipt-fill handoff into a row-by-row bridge verification path. "
    ...
    for row in rows:
        lines.append(
            f"| {row['checklist_order']} | {row['frontier_name']} | {row['readiness_status']} | "
...
def write_outputs(rows: list[dict[str, str]]) -> tuple[Path, Path, Path]:
    ...
    csv_path = tables_dir / "frontier_execution_receipt_queue_handoff_bridge_checklist.csv"
    json_path = tables_dir / "frontier_execution_receipt_queue_handoff_bridge_checklist.json"
    md_path = figures_dir / "frontier_execution_receipt_queue_handoff_bridge_checklist.md"
...
def main() -> None:
    ...
    print(
        "Wrote frontier execution receipt queue handoff bridge checklist CSV: "
        f"{csv_path.relative_to(PROJECT_ROOT)}"
    )
    print(
        "Wrote frontier execution receipt queue handoff bridge checklist JSON: "
        f"{json_path.relative_to(PROJECT_ROOT)}"
    )
    print(
        "Wrote frontier execution receipt queue handoff bridge checklist note: "
