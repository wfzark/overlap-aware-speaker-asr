...
from .io_helpers import readfrom __future__ import annotations
...
def build_section_rows() -> list[dict[str, str]]:
    ...
    # Add boundary detection metrics if available
    boundary_json = PROJECT_ROOT / "results" / "tables" / "separation_phase_boundary.json"
    if boundary_json.exists():
        try:
            boundary_data = read_json(boundary_json)
            if isinstance(boundary_data, dict):
                boundary_metrics = {
                    "boundary_type": str(boundary_data.get("boundary_type", "")),
                    "crossover_ratio": str(boundary_data.get("crossover_ratio", "")),
                    "crossover_ci_lower": str(boundary_data.get("crossover_ci_lower", "")),
                    "crossover_ci_upper": str(boundary_data.get("crossover_ci_upper", "")),
                    "crossover_ci_width": str(boundary_data.get("crossover_ci_width", "")),
                    "below_boundary_help_rate": str(boundary_data.get("below_boundary_help_rate", "")),
                    "above_boundary_help_rate": str(boundary_data.get("above_boundary_help_rate", "")),
                }
                for metric, value in boundary_metrics.items():
                    rows.append(
                        {
                            "section": "separation_phase_boundary",
                            "metric": metric,
                            "value": value,
                            "label": "experimental/frontier",
                        }
...
def build_finding_rows() -> list[dict[str, str]]:
    ...
    # Add boundary detection finding if available
    boundary_json = PROJECT_ROOT / "results" / "tables" / "separation_phase_boundary.json"
    if boundary_json.exists():
        try:
            boundary_data = read_json(boundary_json)
            if isinstance(boundary_data, dict):
                bt = str(boundary_data.get("boundary_type", ""))
                cr = boundary_data.get("crossover_ratio", "")
                ci_l = boundary_data.get("crossover_ci_lower", "")
                ci_u = boundary_data.get("crossover_ci_upper", "")
                ci_w = boundary_data.get("crossover_ci_width", "")
                if bt in ("crosses_to_harmful", "crosses_to_helpful") and cr != "":
                    findings.append({
                        "finding_id": "F4",
                        "source_module": "separation_phase_boundary",
                        "statement": (
                            f"LOWESS+bootstrap boundary detection identifies crossover "
                            f"at r={cr} [{ci_l}, {ci_u}] (width={ci_w}). "
                            f"Boundary type: {bt}."
                        ),
                        "label": "experimental/frontier",
                    })
                    ...
                    findings.append({
                        ...
                        "source_module": "separation_phase_boundary",
                        "statement": (
                            f"Boundary type: {bt}. No clear crossover detected; "
                            f"this may reflect data noise or insufficient sweep density."
                        ),
                        "label": "experimental/frontier",
                    })
        except (json.JSONDecodeError, KeyError):
