...
def assert_closure_preconditions(
    ...
    if str(wave53_receipt.get("execution_status", "")) != "wave53_exploration_baseline_closure_complete":
        raise RuntimeError("Wave53 closure receipt must be complete before Wave54 exploration baseline closure")
    if (
        str(external_validation_receipt.get("execution_status", ""))
        != "wave53_external_validation_narrow_slice_coordination_complete"
    ):
        raise RuntimeError(
            "Wave53 external validation narrow slice coordination must be complete before Wave54 closure"
        )
    if str(demo_wave53_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave53 presentation writeback must be filled before Wave54 closure")
    if str(demo_wave53_fill.get("storyboard_receipt_status", "")) != "wave53_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave53 storyboard receipt must be wave53_presentation_extension_complete before Wave54 closure"
        )
...
def build_closure_rows() -> list[dict[str, str]]:
    return [
        {
            "section_id": "wave53_rollup",
            "headline": "Wave53 closed: external validation narrow slice refresh + demo + exploration closure",
        ...
        },
        {
            ...
            "coordination_note": "Wave54 closure does not record MeetEval official benchmark execution sessions.",
            "result_label": "experimental/frontier",
        },
    ]
...
def build_fill_row(rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave54_exploration_baseline_closure_card",
        "coordination_section_count": str(len(rows)),
        "execution_receipt_status": "wave54_exploration_baseline_closure_complete",
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": (
            "Wave54 exploration+baseline closure rollup after Wave53 chain; stable gold baseline preserved."
        ),
    }
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave54_exploration_baseline_closure_complete",
        "coordination_scope": "wave54_exploration_baseline",
        ...
        "demo_wave53_status": str(demo_wave53_fill.get("storyboard_receipt_status", "")),
        "frontier_go_count": str(frontier_summary.get("go_count", "")),
        "expected_inputs": (
            "Wave53 closure, external validation, demo wave53, frontier summary."
        ),
        "writeback_note": (
            "探索+基线 Wave54 closure writeback; qualitative/demo and experimental/frontier labels preserved."
        ),
    }
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave54 Exploration+Baseline Closure Card",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave54 Exploration+Baseline Closure Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave54 Exploration+Baseline Closure Receipt",
        "",
        "| execution_status | wave53_closure_status | frontier_go_count |",
        "| --- | --- | ---: |",
        (
            f"| {row['execution_status']} | {row['wave53_closure_status']} | {row['frontier_go_count']} |"
        ),
    ]
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave54_exploration_baseline_closure_card.csv"
    card_json = tables_dir / "wave54_exploration_baseline_closure_card.json"
    card_md = figures_dir / "wave54_exploration_baseline_closure_card.md"
    fill_csv = tables_dir / "wave54_exploration_baseline_closure_writeback.csv"
    fill_json = tables_dir / "wave54_exploration_baseline_closure_writeback.json"
    fill_md = figures_dir / "wave54_exploration_baseline_closure_writeback.md"
    receipt_json = tables_dir / "wave54_exploration_baseline_closure_receipt.json"
    receipt_md = figures_dir / "wave54_exploration_baseline_closure_receipt.md"
...
def run_closure_writeback(force: bool = False) -> dict[str, str]:
    ...
    assert_closure_preconditions(wave53_receipt, external_validation_receipt, demo_wave53_fill, frontier_summary)

    receipt_path = PROJECT_ROOT / "results/tables/wave54_exploration_baseline_closure_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict("results/tables/wave54_exploration_baseline_closure_receipt.json")
        if str(existing.get("execution_status", "")) == "wave54_exploration_baseline_closure_complete":
            return {
    ...
    receipt_row = build_receipt_row(wave53_receipt, external_validation_receipt, demo_wave53_fill, frontier_summary)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Write Wave54 exploration+baseline closure after Wave53 chain.")
