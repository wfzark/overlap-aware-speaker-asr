...
def assert_writeback_preconditions(
    ...
    wave16_refresh_receipt: dict[str, Any],
) -> None:
    if str(wave21_receipt.get("execution_status", "")) != "wave21_exploration_baseline_closure_complete":
        raise RuntimeError("Wave21 closure must be complete before LightOverlap diagnostic refresh coordination")
    if str(demo_wave21_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave21 presentation writeback must be filled before LightOverlap coordination")
    if str(demo_wave21_fill.get("storyboard_receipt_status", "")) != "wave21_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave21 storyboard receipt must be wave21_presentation_extension_complete before LightOverlap coordination"
        )
    if str(lightoverlap_receipt.get("execution_status", "")) != "speaker_profile_lightoverlap_diagnostic_coordination_complete":
        raise RuntimeError(
            "Speaker profile LightOverlap diagnostic coordination must be complete before Wave21 refresh"
        )
    if str(wave16_refresh_receipt.get("execution_status", "")) != "wave16_speaker_profile_lightoverlap_diagnostic_coordination_complete":
        raise RuntimeError(
            "Wave16 LightOverlap diagnostic refresh must be complete before Wave21 refresh coordination"
        )
    ...
    return [
        {
            ...
            "headline": "LightOverlap separation-harm diagnostic scope refresh after Wave21 closure",
        ...
        },
        {
            ...
            "coordination_note": "Wave21 refresh does not replace original LightOverlap boundary documentation.",
            "result_label": "experimental/frontier",
        },
        {
            "section_id": "wave16_refresh",
            "headline": "Wave16 LightOverlap diagnostic refresh precedes Wave21 refresh",
        ...
        },
        {
            ...
            "headline": "Wave21 closure defers overlap embedding execution to diagnostic boundaries only",
...
def build_fill_row(rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave21_speaker_profile_lightoverlap_diagnostic_coordination_card",
        "coordination_section_count": str(len(rows)),
        "diagnostic_case_scope": "LightOverlap",
        "candidate_case_scope": "MidOverlap",
        "execution_receipt_status": "wave21_speaker_profile_lightoverlap_diagnostic_coordination_complete",
        "blocker": "attribution_claims_still_blocked",
        "fill_note": (
            "Documented LightOverlap diagnostic refresh after Wave21 closure; "
...
def build_receipt_row(
    ...
    return {
        ...
        "coordination_scope": "wave21_speaker_profile_lightoverlap_diagnostic",
        ...
        "wave16_refresh_status": str(wave16_refresh_receipt.get("execution_status", "")),
        "separation_harm_signal": "separation_hurts_on_lightoverlap",
        "expected_inputs": (
            "Wave21 closure, demo wave21 writeback, Wave8 LightOverlap coordination, and Wave16 refresh."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave21 Speaker Profile LightOverlap Diagnostic Coordination Card (experimental/frontier)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave21 Speaker Profile LightOverlap Diagnostic Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave21 Speaker Profile LightOverlap Diagnostic Coordination Receipt",
        "",
        "| execution_status | wave16_refresh_status | separation_harm_signal |",
        "| --- | --- | --- |",
        (
            f"| {row['execution_status']} | {row['wave16_refresh_status']} | "
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave21_speaker_profile_lightoverlap_diagnostic_coordination_card.csv"
    card_json = tables_dir / "wave21_speaker_profile_lightoverlap_diagnostic_coordination_card.json"
    card_md = figures_dir / "wave21_speaker_profile_lightoverlap_diagnostic_coordination_card.md"
    fill_csv = tables_dir / "wave21_speaker_profile_lightoverlap_diagnostic_coordination_writeback.csv"
    fill_json = tables_dir / "wave21_speaker_profile_lightoverlap_diagnostic_coordination_writeback.json"
    fill_md = figures_dir / "wave21_speaker_profile_lightoverlap_diagnostic_coordination_writeback.md"
    receipt_json = tables_dir / "wave21_speaker_profile_lightoverlap_diagnostic_coordination_receipt.json"
    receipt_md = figures_dir / "wave21_speaker_profile_lightoverlap_diagnostic_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    ...
    wave16_refresh_receipt = load_json_dict(
        "results/tables/wave16_speaker_profile_lightoverlap_diagnostic_coordination_receipt.json"
    )
    assert_writeback_preconditions(
        wave21_receipt, demo_wave21_fill, lightoverlap_receipt, wave16_refresh_receipt
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave21_speaker_profile_lightoverlap_diagnostic_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict(
            "results/tables/wave21_speaker_profile_lightoverlap_diagnostic_coordination_receipt.json"
        )
        if str(existing.get("execution_status", "")) == "wave21_speaker_profile_lightoverlap_diagnostic_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(wave21_receipt, lightoverlap_receipt, wave16_refresh_receipt)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write Wave21 speaker profile LightOverlap diagnostic refresh after Wave21 closure."
    )
