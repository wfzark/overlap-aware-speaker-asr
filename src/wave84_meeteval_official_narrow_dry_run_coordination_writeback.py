...
def assert_writeback_preconditions(
    ...
    if str(wave84_receipt.get("execution_status", "")) != "wave84_exploration_baseline_closure_complete":
        raise RuntimeError("Wave84 closure must be complete before MeetEval official narrow dry-run refresh coordination")
    if str(demo_wave84_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave84 presentation writeback must be filled before MeetEval official coordination")
    if str(demo_wave84_fill.get("storyboard_receipt_status", "")) != "wave84_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave84 storyboard receipt must be wave84_presentation_extension_complete before MeetEval official coordination"
        )
    if (
        ...
        raise RuntimeError(
            "Wave78 MeetEval official narrow dry-run coordination must be complete before Wave84 refresh"
        )
...
def build_coordination_rows() -> list[dict[str, str]]:
    return [
        ...
        {
            "section_id": "wave78_prior_coordination",
            "headline": "Wave78 official MeetEval narrow dry-run refresh remains the prerequisite chain",
            "artifact_anchor": "results/figures/wave78_meeteval_official_narrow_dry_run_coordination_card.md",
            "coordination_note": "Wave84 refresh does not replace Wave78 official MeetEval boundary documentation.",
            "result_label": "experimental/frontier",
        },
        {
...
def build_fill_row(rows: list[dict[str, str]], official_dry_run_case_count: str) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave84_meeteval_official_narrow_dry_run_coordination_card",
        "coordination_section_count": str(len(rows)),
        "official_dry_run_case_count": official_dry_run_case_count,
        "execution_receipt_status": "wave84_meeteval_official_narrow_dry_run_coordination_complete",
...
def build_receipt_row(
    ...
    return {
        ...
        "expected_inputs": (
            "Wave84 closure, demo wave78, Wave72 official coordination, Wave17 character-level refresh, official execution."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave84 MeetEval Official Narrow Dry-Run Coordination Card (experimental/frontier)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave84 MeetEval Official Narrow Dry-Run Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave84 MeetEval Official Narrow Dry-Run Coordination Receipt",
        "",
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave84_meeteval_official_narrow_dry_run_coordination_card.csv"
    card_json = tables_dir / "wave84_meeteval_official_narrow_dry_run_coordination_card.json"
    card_md = figures_dir / "wave84_meeteval_official_narrow_dry_run_coordination_card.md"
    fill_csv = tables_dir / "wave84_meeteval_official_narrow_dry_run_coordination_writeback.csv"
    fill_json = tables_dir / "wave84_meeteval_official_narrow_dry_run_coordination_writeback.json"
    fill_md = figures_dir / "wave84_meeteval_official_narrow_dry_run_coordination_writeback.md"
    receipt_json = tables_dir / "wave84_meeteval_official_narrow_dry_run_coordination_receipt.json"
    receipt_md = figures_dir / "wave84_meeteval_official_narrow_dry_run_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    ...
    assert_writeback_preconditions(
        wave84_receipt, demo_wave84_fill, prior_official_receipt, character_refresh_receipt
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave84_meeteval_official_narrow_dry_run_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict("results/tables/wave84_meeteval_official_narrow_dry_run_coordination_receipt.json")
        if str(existing.get("execution_status", "")) == "wave84_meeteval_official_narrow_dry_run_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(
        wave84_receipt, prior_official_receipt, character_refresh_receipt, official_count
    )
