...
def load_timing_summary() -> dict[str, Any]:
    summary_path = (
...
def build_handoff_row(summary: dict[str, Any]) -> dict[str, str]:
    ...
    return {
        ...
        "granularity_diagnostic_target": (
            "results/figures/meeteval_cpwer_alignment_drift_segment_granularity_diagnostic_summary.md"
        ),
        "handoff_goal": (
            f"Run a narrow per-speaker segment granularity diagnostic for {case_id} after the timing drift finding."
        ),
        "expected_evidence": (
            "results/tables/meeteval_cpwer_alignment_drift_segment_granularity_diagnostic_receipt.json"
        ),
        "handoff_note": (
            "experimental/frontier timing handoff only; reconciled alignment and cpWER execution remain pending."
        ),
    }
...
def build_handoff_lines(row: dict[str, str]) -> list[str]:
    lines = [
        ...
        "",
        "This generated handoff turns the per-speaker timing diagnostic into the next narrow MeetEval frontier step. "
        ...
        (
            ...
            f"{row['dominant_blocker']} | {row['granularity_diagnostic_target']} | {row['handoff_goal']} | "
    ...
    return [
        {
            ...
            "handoff_scope": "single_drift_case_timing_to_granularity",
            "case_id": str(handoff_row.get("case_id", "")),
            "writeback_note": (
                "Timing diagnostic handoff documented for coordination; "
                "per-speaker granularity diagnostic and cpWER execution remain pending."
            ),
        }
    ]
...
def build_handoff_receipt_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# MeetEval cpWER Alignment Drift Segment Timing Diagnostic Handoff Receipt",
        "",
        "This receipt records the timing diagnostic handoff writeback. It does not claim cpWER execution.",
        "",
...
def write_outputs(
    ...
    csv_path = tables_dir / "meeteval_cpwer_alignment_drift_segment_timing_diagnostic_handoff.csv"
    json_path = tables_dir / "meeteval_cpwer_alignment_drift_segment_timing_diagnostic_handoff.json"
    md_path = figures_dir / "meeteval_cpwer_alignment_drift_segment_timing_diagnostic_handoff.md"
    receipt_json_path = tables_dir / "meeteval_cpwer_alignment_drift_segment_timing_diagnostic_handoff_receipt.json"
    receipt_md_path = figures_dir / "meeteval_cpwer_alignment_drift_segment_timing_diagnostic_handoff_receipt.md"
...
def main() -> None:
    ...
    print(
        "Wrote MeetEval cpWER alignment drift segment timing diagnostic handoff CSV: "
        f"{csv_path.relative_to(PROJECT_ROOT)}"
    )
    print(
        "Wrote MeetEval cpWER alignment drift segment timing diagnostic handoff JSON: "
        f"{json_path.relative_to(PROJECT_ROOT)}"
    )
    print(
        "Wrote MeetEval cpWER alignment drift segment timing diagnostic handoff note: "
        f"{md_path.relative_to(PROJECT_ROOT)}"
    )
    print(
        "Wrote MeetEval cpWER alignment drift segment timing diagnostic handoff receipt JSON: "
        f"{receipt_json_path.relative_to(PROJECT_ROOT)}"
    )
    print(
        "Wrote MeetEval cpWER alignment drift segment timing diagnostic handoff receipt note: "
