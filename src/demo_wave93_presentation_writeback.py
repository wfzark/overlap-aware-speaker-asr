...
from .demo_wave92_presentation_writeback import build_extended_polish_rows as build_wave92_polish_rows
...
def assert_extension_preconditions(
    ...
    wave92_fill: dict[str, Any],
) -> None:
    if str(wave93_receipt.get("execution_status", "")) != "wave93_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave93 exploration baseline closure must be complete before wave93 demo extension"
        )
    if str(wave92_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave92 presentation writeback must be filled before wave93 extension")
    if str(wave92_fill.get("storyboard_receipt_status", "")) != "wave92_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave92 storyboard receipt must be wave92_presentation_extension_complete before wave93 extension"
        )


def build_wave93_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave93",
        "headline": "Wave93 exploration+baseline closure and HeavyOverlap speaker-profile diagnostic refresh speaker-profile diagnostic refresh boundary",
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave92_polish_rows()
    if any(row["section_id"] == "frontier_wave93" for row in rows):
        return rows
    return rows + [build_wave93_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave93_presentation_extension_complete",
        "storyboard_scope": "wave93_storyboard_extension",
        "expected_inputs": "Wave93 exploration+baseline closure card.",
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave93_presentation_extension_complete",
        "walkthrough_scope": "wave93_walkthrough_extension",
        "expected_inputs": "Wave93 exploration+baseline closure card.",
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave93_presentation_polish_extension",
        "storyboard_receipt_status": "wave93_presentation_extension_complete",
        "walkthrough_receipt_status": "wave93_presentation_extension_complete",
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave93_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave93_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave93_presentation_writeback.md"
    ...
    fill_md.write_text(
        "\n".join(
            [
                "# Demo Wave93 Presentation Writeback",
                "",
...
def run_wave93_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave93_receipt = load_json_dict("results/tables/wave93_exploration_baseline_closure_receipt.json")
    wave92_fill = load_json_dict("results/tables/demo_wave92_presentation_writeback.json")
    assert_extension_preconditions(wave93_receipt, wave92_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave93_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave93_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave93 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave93 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave93_presentation_writeback(force=args.force)
