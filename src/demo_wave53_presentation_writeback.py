...
from .demo_wave52_presentation_writeback import build_extended_polish_rows as build_wave52_polish_rows
...
def assert_extension_preconditions(
    ...
    wave52_fill: dict[str, Any],
) -> None:
    if str(wave53_receipt.get("execution_status", "")) != "wave53_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave53 exploration baseline closure must be complete before wave53 demo extension"
        )
    if str(wave52_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave52 presentation writeback must be filled before wave53 extension")
    if str(wave52_fill.get("storyboard_receipt_status", "")) != "wave52_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave52 storyboard receipt must be wave52_presentation_extension_complete before wave53 extension"
        )


def build_wave53_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave53",
        "headline": "Wave53 exploration+baseline closure and External validation diagnostic refresh boundary",
        "artifact_anchor": "results/figures/wave53_exploration_baseline_closure_card.md",
        "presentation_note": (
            "Show Wave53 closure card only; External validation speaker-profile diagnostic refresh remains "
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave52_polish_rows()
    if any(row["section_id"] == "frontier_wave53" for row in rows):
        return rows
    return rows + [build_wave53_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave53_presentation_extension_complete",
        "storyboard_scope": "wave53_storyboard_extension",
        "expected_inputs": "Wave53 exploration+baseline closure card.",
        "expected_outputs": "Wave53 storyboard extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave53 storyboard extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave53_presentation_extension_complete",
        "walkthrough_scope": "wave53_walkthrough_extension",
        "expected_inputs": "Wave53 exploration+baseline closure card.",
        "expected_outputs": "Wave53 walkthrough extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave53 walkthrough extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave53_presentation_polish_extension",
        "storyboard_receipt_status": "wave53_presentation_extension_complete",
        "walkthrough_receipt_status": "wave53_presentation_extension_complete",
        "polish_section_count": str(len(polish_rows)),
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": "Extended presentation polish card with Wave53 exploration+baseline closure section.",
    }
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave53_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave53_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave53_presentation_writeback.md"
    ...
    fill_md.write_text(
        "\n".join(
            [
                "# Demo Wave53 Presentation Writeback",
                "",
...
def run_wave53_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave53_receipt = load_json_dict("results/tables/wave53_exploration_baseline_closure_receipt.json")
    wave52_fill = load_json_dict("results/tables/demo_wave52_presentation_writeback.json")
    assert_extension_preconditions(wave53_receipt, wave52_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave53_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave53_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave53 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave53 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave53_presentation_writeback(force=args.force)
