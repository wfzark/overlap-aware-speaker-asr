...
def assert_writeback_preconditions(
    ...
    wave117_refresh_receipt: dict[str, Any],
    ...
    if str(wave123_receipt.get("execution_status", "")) != "wave123_exploration_baseline_closure_complete":
        raise RuntimeError("Wave123 closure must be complete before HeavyOverlap diagnostic refresh coordination")
    if str(demo_wave123_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave123 presentation writeback must be filled before HeavyOverlap coordination")
    if str(demo_wave123_fill.get("storyboard_receipt_status", "")) != "wave123_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave123 storyboard receipt must be wave123_presentation_extension_complete before HeavyOverlap coordination"
        )
    if str(heavyoverlap_receipt.get("execution_status", "")) != "speaker_profile_heavyoverlap_diagnostic_coordination_complete":
        raise RuntimeError(
            "Speaker profile HeavyOverlap diagnostic coordination must be complete before Wave117 refresh"
        )
    if (
        str(wave117_refresh_receipt.get("execution_status", ""))
        != "wave117_speaker_profile_heavyoverlap_diagnostic_coordination_complete"
    ):
        raise RuntimeError(
            "Wave117 HeavyOverlap diagnostic refresh must be complete before Wave123 refresh coordination"
        )
    if (
        ...
        raise RuntimeError(
            "Wave122 MidOverlap diagnostic refresh must be complete before HeavyOverlap refresh coordination"
        )
    ...
    return [
        {
            ...
            "headline": "HeavyOverlap separation-benefit diagnostic scope refresh after Wave123 closure",
        ...
        {
            "section_id": "wave117_refresh",
            "headline": "Wave117 HeavyOverlap diagnostic refresh precedes Wave123 refresh",
        ...
        },
        {
            ...
            "headline": "Wave122 MidOverlap diagnostic refresh precedes HeavyOverlap refresh",
        ...
        },
        {
            ...
            "headline": "Wave123 closure defers overlap embedding execution to diagnostic boundaries only",
...
def build_fill_row(rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave123_speaker_profile_heavyoverlap_diagnostic_coordination_card",
        "coordination_section_count": str(len(rows)),
        "diagnostic_case_scope": "HeavyOverlap",
        "candidate_case_scope": "OppositeOverlap",
        "execution_receipt_status": "wave123_speaker_profile_heavyoverlap_diagnostic_coordination_complete",
        "blocker": "attribution_claims_still_blocked",
        "fill_note": (
            "Documented HeavyOverlap diagnostic refresh after Wave123 closure; "
...
def build_receipt_row(
    ...
    return {
        ...
        "coordination_scope": "wave123_speaker_profile_heavyoverlap_diagnostic",
        ...
        "wave117_refresh_status": str(wave117_refresh_receipt.get("execution_status", "")),
        ...
        "expected_inputs": (
            "Wave123 closure, demo wave123, Wave12 HeavyOverlap, Wave117 refresh, and Wave122 MidOverlap refresh."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave123 Speaker Profile HeavyOverlap Diagnostic Coordination Card (experimental/frontier)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave123 Speaker Profile HeavyOverlap Diagnostic Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave123 Speaker Profile HeavyOverlap Diagnostic Coordination Receipt",
        "",
        "| execution_status | wave117_refresh_status | midoverlap_refresh_status |",
        "| --- | --- | --- |",
        (
            f"| {row['execution_status']} | {row['wave117_refresh_status']} | "
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave123_speaker_profile_heavyoverlap_diagnostic_coordination_card.csv"
    card_json = tables_dir / "wave123_speaker_profile_heavyoverlap_diagnostic_coordination_card.json"
    card_md = figures_dir / "wave123_speaker_profile_heavyoverlap_diagnostic_coordination_card.md"
    fill_csv = tables_dir / "wave123_speaker_profile_heavyoverlap_diagnostic_coordination_writeback.csv"
    fill_json = tables_dir / "wave123_speaker_profile_heavyoverlap_diagnostic_coordination_writeback.json"
    fill_md = figures_dir / "wave123_speaker_profile_heavyoverlap_diagnostic_coordination_writeback.md"
    receipt_json = tables_dir / "wave123_speaker_profile_heavyoverlap_diagnostic_coordination_receipt.json"
    receipt_md = figures_dir / "wave123_speaker_profile_heavyoverlap_diagnostic_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    ...
    wave117_refresh_receipt = load_json_dict(
    ...
    assert_writeback_preconditions(
        ...
        wave117_refresh_receipt,
        midoverlap_refresh_receipt,
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave123_speaker_profile_heavyoverlap_diagnostic_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict(
            "results/tables/wave123_speaker_profile_heavyoverlap_diagnostic_coordination_receipt.json"
        )
        if str(existing.get("execution_status", "")) == "wave123_speaker_profile_heavyoverlap_diagnostic_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(
        wave123_receipt, heavyoverlap_receipt, wave117_refresh_receipt, midoverlap_refresh_receipt
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write Wave81 speaker profile HeavyOverlap diagnostic refresh after Wave123 closure."
    )
