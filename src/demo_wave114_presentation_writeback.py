...
from .demo_wave113_presentation_writeback import build_extended_polish_rows as build_wave113_polish_rows
...
def assert_extension_preconditions(
    ...
    wave113_fill: dict[str, Any],
) -> None:
    if str(wave114_receipt.get("execution_status", "")) != "wave114_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave114 exploration baseline closure must be complete before wave114 demo extension"
        )
    if str(wave113_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave113 presentation writeback must be filled before wave114 extension")
    if str(wave113_fill.get("storyboard_receipt_status", "")) != "wave113_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave113 storyboard receipt must be wave113_presentation_extension_complete before wave114 extension"
        )


def build_wave114_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave114",
        "headline": "Wave113 exploration+baseline closure after Wave113 external validation chain boundary",
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave113_polish_rows()
    if any(row["section_id"] == "frontier_wave114" for row in rows):
        return rows
    return rows + [build_wave114_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave114_presentation_extension_complete",
        "storyboard_scope": "wave114_storyboard_extension",
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave114_presentation_extension_complete",
        "walkthrough_scope": "wave114_walkthrough_extension",
        "expected_inputs": "Wave104 exploration+baseline closure card.",
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave114_presentation_polish_extension",
        "storyboard_receipt_status": "wave114_presentation_extension_complete",
        "walkthrough_receipt_status": "wave114_presentation_extension_complete",
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave114_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave114_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave114_presentation_writeback.md"
...
def run_wave114_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave114_receipt = load_json_dict("results/tables/wave114_exploration_baseline_closure_receipt.json")
    wave113_fill = load_json_dict("results/tables/demo_wave113_presentation_writeback.json")
    assert_extension_preconditions(wave114_receipt, wave113_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave114_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave114_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave114 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave114 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave114_presentation_writeback(force=args.force)
