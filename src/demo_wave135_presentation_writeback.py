...
from .demo_wave134_presentation_writeback import build_extended_polish_rows as build_wave134_polish_rows
...
def assert_extension_preconditions(
    ...
    wave134_fill: dict[str, Any],
) -> None:
    if str(wave135_receipt.get("execution_status", "")) != "wave135_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave134 exploration baseline closure must be complete before wave135 demo extension"
        )
    if str(wave134_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave134 presentation writeback must be filled before wave135 extension")
    if str(wave134_fill.get("storyboard_receipt_status", "")) != "wave134_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave134 storyboard receipt must be wave134_presentation_extension_complete before wave135 extension"
        )


def build_wave135_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave135",
        "headline": "Wave134 exploration+baseline closure after Wave134 LightOverlap chain boundary",
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave134_polish_rows()
    if any(row["section_id"] == "frontier_wave135" for row in rows):
        return rows
    return rows + [build_wave135_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave135_presentation_extension_complete",
        "storyboard_scope": "wave135_storyboard_extension",
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave135_presentation_extension_complete",
        "walkthrough_scope": "wave135_walkthrough_extension",
        "expected_inputs": "Wave104 exploration+baseline closure card.",
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave135_presentation_polish_extension",
        "storyboard_receipt_status": "wave135_presentation_extension_complete",
        "walkthrough_receipt_status": "wave135_presentation_extension_complete",
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave135_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave135_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave135_presentation_writeback.md"
...
def run_wave135_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave135_receipt = load_json_dict("results/tables/wave135_exploration_baseline_closure_receipt.json")
    wave134_fill = load_json_dict("results/tables/demo_wave134_presentation_writeback.json")
    assert_extension_preconditions(wave135_receipt, wave134_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave135_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave135_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave135 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave135_presentation_writeback(force=args.force)
