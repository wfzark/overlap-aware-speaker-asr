...
def build_dashboard_row(
    ...
    if not operator_brief or not milestone_card:
        return {}
    current_first_frontier = str(operator_brief.get("ready_frontier", ""))
    blocked_frontier = str(operator_brief.get("blocked_frontier", ""))
    ...
    return {
        ...
        "dashboard_note": (
            f"{current_first_frontier or 'The ready frontier'} leads the status/handoff subchain while "
...
def build_dashboard_lines(row: dict[str, str]) -> list[str]:
    return [
        ...
        "",
        "This generated dashboard summarizes the current top-level status/handoff subchain at a glance. "
...
def write_outputs(row: dict[str, str]) -> tuple[Path, Path, Path]:
    ...
    csv_path = tables_dir / "frontier_operator_next_action_status_handoff_completion_dashboard.csv"
    json_path = tables_dir / "frontier_operator_next_action_status_handoff_completion_dashboard.json"
    md_path = figures_dir / "frontier_operator_next_action_status_handoff_completion_dashboard.md"
...
def main() -> None:
    ...
    if not row:
        print("Operator brief or milestone card not found; completion dashboard not written.")
        return
    csv_path, json_path, md_path = write_outputs(row)
    print(
        "Wrote frontier operator next-action status handoff completion dashboard CSV: "
        f"{csv_path.relative_to(PROJECT_ROOT)}"
    )
    print(
        "Wrote frontier operator next-action status handoff completion dashboard JSON: "
        f"{json_path.relative_to(PROJECT_ROOT)}"
    )
    print(
        "Wrote frontier operator next-action status handoff completion dashboard note: "
