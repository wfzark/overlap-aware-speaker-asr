...
RECEIPT_COLUMNS = [
    ...
    "wave79_lightoverlap_refresh_status",
...
def assert_writeback_preconditions(
    ...
    wave79_lightoverlap_receipt: dict[str, Any],
    lightoverlap_receipt: dict[str, Any],
) -> None:
    if str(wave85_receipt.get("execution_status", "")) != "wave85_exploration_baseline_closure_complete":
        raise RuntimeError("Wave85 closure must be complete before LightOverlap diagnostic refresh coordination")
    if str(demo_wave85_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave85 presentation writeback must be filled before LightOverlap coordination")
    if str(demo_wave85_fill.get("storyboard_receipt_status", "")) != "wave85_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave85 storyboard receipt must be wave85_presentation_extension_complete before LightOverlap coordination"
        )
    if (
        str(wave79_lightoverlap_receipt.get("execution_status", ""))
        != "wave79_speaker_profile_lightoverlap_diagnostic_coordination_complete"
    ):
        raise RuntimeError(
            "Wave79 LightOverlap diagnostic refresh must be complete before Wave85 refresh coordination"
        )
    if str(lightoverlap_receipt.get("execution_status", "")) != "speaker_profile_lightoverlap_diagnostic_coordination_complete":
        raise RuntimeError(
            "Speaker profile LightOverlap diagnostic coordination must be complete before Wave85 refresh"
        )
    ...
    return [
        {
            ...
            "headline": "LightOverlap separation-harm diagnostic scope refresh after Wave85 closure",
        ...
        {
            "section_id": "wave79_refresh",
            "headline": "Wave79 LightOverlap diagnostic refresh precedes Wave85 refresh",
        ...
        },
        {
            ...
            "headline": "Wave85 closure defers overlap embedding execution to diagnostic boundaries only",
...
def build_fill_row(rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave85_speaker_profile_lightoverlap_diagnostic_coordination_card",
        "coordination_section_count": str(len(rows)),
        "diagnostic_case_scope": "LightOverlap",
        "candidate_case_scope": "MidOverlap",
        "execution_receipt_status": "wave85_speaker_profile_lightoverlap_diagnostic_coordination_complete",
        "blocker": "attribution_claims_still_blocked",
        "fill_note": (
            "Documented LightOverlap diagnostic refresh after Wave85 closure; "
...
def build_receipt_row(
    ...
    return {
        ...
        "wave79_lightoverlap_refresh_status": str(wave79_lightoverlap_receipt.get("execution_status", "")),
        ...
        "expected_inputs": (
            "Wave85 closure, demo wave79 writeback, Wave79 LightOverlap refresh, and Wave8 LightOverlap coordination."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave85 Speaker Profile LightOverlap Diagnostic Coordination Card (experimental/frontier)",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave85 Speaker Profile LightOverlap Diagnostic Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave85 Speaker Profile LightOverlap Diagnostic Coordination Receipt",
        "",
        "| execution_status | wave79_lightoverlap_refresh_status | separation_harm_signal |",
        "| --- | --- | --- |",
        (
            f"| {row['execution_status']} | {row['wave79_lightoverlap_refresh_status']} | "
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave85_speaker_profile_lightoverlap_diagnostic_coordination_card.csv"
    card_json = tables_dir / "wave85_speaker_profile_lightoverlap_diagnostic_coordination_card.json"
    card_md = figures_dir / "wave85_speaker_profile_lightoverlap_diagnostic_coordination_card.md"
    fill_csv = tables_dir / "wave85_speaker_profile_lightoverlap_diagnostic_coordination_writeback.csv"
    fill_json = tables_dir / "wave85_speaker_profile_lightoverlap_diagnostic_coordination_writeback.json"
    fill_md = figures_dir / "wave85_speaker_profile_lightoverlap_diagnostic_coordination_writeback.md"
    receipt_json = tables_dir / "wave85_speaker_profile_lightoverlap_diagnostic_coordination_receipt.json"
    receipt_md = figures_dir / "wave85_speaker_profile_lightoverlap_diagnostic_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    ...
    wave79_lightoverlap_receipt = load_json_dict(
    ...
    assert_writeback_preconditions(
        wave85_receipt, demo_wave85_fill, wave79_lightoverlap_receipt, lightoverlap_receipt
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave85_speaker_profile_lightoverlap_diagnostic_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict(
            "results/tables/wave85_speaker_profile_lightoverlap_diagnostic_coordination_receipt.json"
        )
        if str(existing.get("execution_status", "")) == "wave85_speaker_profile_lightoverlap_diagnostic_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(wave85_receipt, wave79_lightoverlap_receipt, lightoverlap_receipt)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write Wave79 speaker profile LightOverlap diagnostic refresh after Wave85 closure."
    )
