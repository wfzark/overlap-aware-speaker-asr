...
from .demo_wave7_presentation_writeback import build_extended_polish_rows as build_wave7_polish_rows
...
def assert_extension_preconditions(
    ...
    wave7_fill: dict[str, Any],
) -> None:
    if str(wave8_receipt.get("execution_status", "")) != "wave8_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave8 exploration baseline closure must be complete before wave8 demo extension"
        )
    if str(demo_summary.get("overall_state", "")) != "presentation_wave7_extension_complete":
        raise RuntimeError(
            f"Demo go/no-go must be presentation_wave7_extension_complete; got {demo_summary.get('overall_state', 'missing')!r}"
        )
    if str(wave7_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave7 presentation writeback must be filled before wave8 extension")


def build_wave8_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave8",
        "headline": "Wave8 exploration+baseline closure and manifest boundary rollup",
        "artifact_anchor": "results/figures/wave8_exploration_baseline_closure_card.md",
        "presentation_note": (
            "Show Wave8 closure card only; speaker overlap diagnostics and benchmark timing remain "
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave7_polish_rows()
    if any(row["section_id"] == "frontier_wave8" for row in rows):
        return rows
    return rows + [build_wave8_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave8_presentation_extension_complete",
        "storyboard_scope": "wave8_storyboard_extension",
        "expected_inputs": "Wave8 exploration+baseline closure card.",
        "expected_outputs": "Wave8 storyboard extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave8 storyboard extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave8_presentation_extension_complete",
        "walkthrough_scope": "wave8_walkthrough_extension",
        "expected_inputs": "Wave8 exploration+baseline closure card.",
        "expected_outputs": "Wave8 walkthrough extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave8 walkthrough extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave8_presentation_polish_extension",
        "storyboard_receipt_status": "wave8_presentation_extension_complete",
        "walkthrough_receipt_status": "wave8_presentation_extension_complete",
        "polish_section_count": str(len(polish_rows)),
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": "Extended presentation polish card with Wave8 exploration+baseline closure section.",
    }
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave8_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave8_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave8_presentation_writeback.md"
    ...
    fill_md.write_text(
        "\n".join(
            [
                "# Demo Wave8 Presentation Writeback",
                "",
...
def run_wave8_presentation_writeback(force: bool = False) -> dict[str, str]:
    ...
    wave7_fill = load_json_dict("results/tables/demo_wave7_presentation_writeback.json")
    assert_extension_preconditions(wave8_receipt, demo_summary, wave7_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave8_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave8_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave8 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave8 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave8_presentation_writeback(force=args.force)
