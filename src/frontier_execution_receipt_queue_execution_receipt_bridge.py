...
def write_outputs(row: dict[str, str]) -> tuple[Path, Path, Path]:
    ...
    csv_path = tables_dir / "frontier_execution_receipt_queue_execution_receipt_bridge.csv"
    json_path = tables_dir / "frontier_execution_receipt_queue_execution_receipt_bridge.json"
    md_path = figures_dir / "frontier_execution_receipt_queue_execution_receipt_bridge.md"
...
def main() -> None:
    ...
    if not row:
        print("Receipt queue evidence receipt not found; execution receipt bridge not written.")
        return
    csv_path, json_path, md_path = write_outputs(row)
    print(
        "Wrote frontier execution receipt queue execution receipt bridge CSV: "
        f"{csv_path.relative_to(PROJECT_ROOT)}"
    )
    print(
        "Wrote frontier execution receipt queue execution receipt bridge JSON: "
        f"{json_path.relative_to(PROJECT_ROOT)}"
    )
    print(
        "Wrote frontier execution receipt queue execution receipt bridge note: "
