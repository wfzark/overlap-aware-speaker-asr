...
from .demo_wave13_presentation_writeback import build_extended_polish_rows as build_wave13_polish_rows
...
def assert_extension_preconditions(
    ...
    wave13_fill: dict[str, Any],
) -> None:
    if str(wave14_receipt.get("execution_status", "")) != "wave14_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave14 exploration baseline closure must be complete before wave14 demo extension"
        )
    if str(wave13_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave13 presentation writeback must be filled before wave14 extension")
    if str(wave13_fill.get("storyboard_receipt_status", "")) != "wave13_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave13 storyboard receipt must be wave13_presentation_extension_complete before wave14 extension"
        )


def build_wave14_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave14",
        "headline": "Wave14 exploration+baseline closure and phase4 gate coordination boundary",
        "artifact_anchor": "results/figures/wave14_exploration_baseline_closure_card.md",
        "presentation_note": (
            "Show Wave14 closure card only; phase4 gate and OppositeOverlap diagnostic remain "
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave13_polish_rows()
    if any(row["section_id"] == "frontier_wave14" for row in rows):
        return rows
    return rows + [build_wave14_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave14_presentation_extension_complete",
        "storyboard_scope": "wave14_storyboard_extension",
        "expected_inputs": "Wave14 exploration+baseline closure card.",
        "expected_outputs": "Wave14 storyboard extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave14 storyboard extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave14_presentation_extension_complete",
        "walkthrough_scope": "wave14_walkthrough_extension",
        "expected_inputs": "Wave14 exploration+baseline closure card.",
        "expected_outputs": "Wave14 walkthrough extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave14 walkthrough extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave14_presentation_polish_extension",
        "storyboard_receipt_status": "wave14_presentation_extension_complete",
        "walkthrough_receipt_status": "wave14_presentation_extension_complete",
        "polish_section_count": str(len(polish_rows)),
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": "Extended presentation polish card with Wave14 exploration+baseline closure section.",
    }
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave14_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave14_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave14_presentation_writeback.md"
    ...
    fill_md.write_text(
        "\n".join(
            [
                "# Demo Wave14 Presentation Writeback",
                "",
...
def run_wave14_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave14_receipt = load_json_dict("results/tables/wave14_exploration_baseline_closure_receipt.json")
    wave13_fill = load_json_dict("results/tables/demo_wave13_presentation_writeback.json")
    assert_extension_preconditions(wave14_receipt, wave13_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave14_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave14_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave14 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave14 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave14_presentation_writeback(force=args.force)
