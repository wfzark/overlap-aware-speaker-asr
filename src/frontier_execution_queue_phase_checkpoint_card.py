...
def build_phase_checkpoint_row(runbook: dict[str, str]) -> dict[str, str]:
    ...
    return {
        ...
        "checkpoint_note": (
            ...
            "before advancing the execution queue stack. No benchmark execution is claimed."
        ),
    }
...
def build_phase_checkpoint_lines(row: dict[str, str]) -> list[str]:
    return [
        ...
        "",
        "This generated checkpoint card shows the per-phase completion signal for the current execution queue step. "
...
def write_outputs(row: dict[str, str]) -> tuple[Path, Path, Path]:
    ...
    csv_path = tables_dir / "frontier_execution_queue_phase_checkpoint_card.csv"
    json_path = tables_dir / "frontier_execution_queue_phase_checkpoint_card.json"
    md_path = figures_dir / "frontier_execution_queue_phase_checkpoint_card.md"
...
def main() -> None:
    ...
    if not row:
        print("Execution queue runbook card not found; phase checkpoint card not written.")
        return
    csv_path, json_path, md_path = write_outputs(row)
    print(f"Wrote frontier execution queue phase checkpoint card CSV: {csv_path.relative_to(PROJECT_ROOT)}")
    print(f"Wrote frontier execution queue phase checkpoint card JSON: {json_path.relative_to(PROJECT_ROOT)}")
    print(f"Wrote frontier execution queue phase checkpoint card note: {md_path.relative_to(PROJECT_ROOT)}")
