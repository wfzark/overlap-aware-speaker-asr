...
def assert_closure_preconditions(
    ...
    demo_wave9_fill: dict[str, Any],
    frontier_summary: dict[str, Any],
) -> None:
    if str(wave9_receipt.get("execution_status", "")) != "wave9_exploration_baseline_closure_complete":
        raise RuntimeError("Wave9 closure receipt must be complete before Wave10 exploration baseline closure")
    if str(phase1_receipt.get("execution_status", "")) != "cascade_benchmark_phase1_gate_coordination_complete":
        raise RuntimeError("Cascade benchmark phase1 gate coordination must be complete before Wave10 closure")
    if str(midoverlap_receipt.get("execution_status", "")) != "speaker_profile_midoverlap_diagnostic_coordination_complete":
        raise RuntimeError("Speaker profile MidOverlap diagnostic coordination must be complete before Wave10 closure")
    if str(demo_wave9_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave9 presentation writeback must be filled before Wave10 closure")
    ...
    return [
        {
            ...
            "headline": "Wave9 closed: MidOverlap diagnostic + phase1 gate + demo + exploration closure",
        ...
        },
        {
            ...
            "headline": "All gold-case speaker diagnostic scopes coordinated",
            "artifact_anchor": "results/figures/speaker_profile_midoverlap_diagnostic_coordination_card.md",
            "coordination_note": "Diagnostic coordination only; attribution claims still blocked.",
            "result_label": "experimental/frontier",
        },
        {
            "section_id": "phase1_gate_boundary",
            "headline": "Benchmark phase1 gate coordinated; controlled timing not executed",
            "artifact_anchor": "results/figures/cascade_benchmark_phase1_gate_coordination_card.md",
            "coordination_note": "phase1_gold_runtime_foundation template_only; hardware metadata TODO.",
            "result_label": "experimental/frontier",
        },
        {
        ...
        },
        {
            ...
            "coordination_note": "Wave10 closure does not record controlled hardware benchmark sessions.",
            "result_label": "experimental/frontier",
        },
    ]
...
def build_fill_row(rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave10_exploration_baseline_closure_card",
        "coordination_section_count": str(len(rows)),
        "execution_receipt_status": "wave10_exploration_baseline_closure_complete",
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": (
            "Wave10 exploration+baseline closure rollup after Wave9 chain; stable gold baseline preserved."
        ),
    }
...
def build_receipt_row(
    ...
    return {
        "execution_status": "wave10_exploration_baseline_closure_complete",
        "coordination_scope": "wave10_exploration_baseline",
        ...
        "expected_inputs": (
            "Wave9 closure, phase1 gate coordination, MidOverlap diagnostic, demo wave9, frontier summary."
        ),
        "writeback_note": (
            "探索+基线 Wave10 closure writeback; qualitative/demo and experimental/frontier labels preserved."
        ),
    }
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# Wave10 Exploration+Baseline Closure Card",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave10 Exploration+Baseline Closure Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# Wave10 Exploration+Baseline Closure Receipt",
        "",
        "| execution_status | wave9_closure_status | frontier_go_count |",
        "| --- | --- | ---: |",
        (
            f"| {row['execution_status']} | {row['wave9_closure_status']} | {row['frontier_go_count']} |"
        ),
    ]
...
def write_outputs(
    ...
    card_csv = tables_dir / "wave10_exploration_baseline_closure_card.csv"
    card_json = tables_dir / "wave10_exploration_baseline_closure_card.json"
    card_md = figures_dir / "wave10_exploration_baseline_closure_card.md"
    fill_csv = tables_dir / "wave10_exploration_baseline_closure_writeback.csv"
    fill_json = tables_dir / "wave10_exploration_baseline_closure_writeback.json"
    fill_md = figures_dir / "wave10_exploration_baseline_closure_writeback.md"
    receipt_json = tables_dir / "wave10_exploration_baseline_closure_receipt.json"
    receipt_md = figures_dir / "wave10_exploration_baseline_closure_receipt.md"
...
def run_closure_writeback(force: bool = False) -> dict[str, str]:
    ...
    demo_wave9_fill = load_json_dict("results/tables/demo_wave9_presentation_writeback.json")
    frontier_summary = load_json_dict("results/tables/frontier_go_no_go_summary.json")
    assert_closure_preconditions(
        wave9_receipt, phase1_receipt, midoverlap_receipt, demo_wave9_fill, frontier_summary
    )

    receipt_path = PROJECT_ROOT / "results/tables/wave10_exploration_baseline_closure_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict("results/tables/wave10_exploration_baseline_closure_receipt.json")
        if str(existing.get("execution_status", "")) == "wave10_exploration_baseline_closure_complete":
            return {
    ...
    receipt_row = build_receipt_row(wave9_receipt, phase1_receipt, midoverlap_receipt, frontier_summary)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Write Wave10 exploration+baseline closure after Wave9 chain.")
