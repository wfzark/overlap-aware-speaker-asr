...
from .demo_wave25_presentation_writeback import build_extended_polish_rows as build_wave25_polish_rows
...
def assert_extension_preconditions(
    ...
    wave25_fill: dict[str, Any],
) -> None:
    if str(wave26_receipt.get("execution_status", "")) != "wave26_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave26 exploration baseline closure must be complete before wave26 demo extension"
        )
    if str(wave25_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave25 presentation writeback must be filled before wave26 extension")
    if str(wave25_fill.get("storyboard_receipt_status", "")) != "wave25_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave25 storyboard receipt must be wave25_presentation_extension_complete before wave26 extension"
        )


def build_wave26_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave26",
        "headline": "Wave26 exploration+baseline closure and MeetEval official dry-run refresh boundary",
        "artifact_anchor": "results/figures/wave26_exploration_baseline_closure_card.md",
        "presentation_note": (
            "Show Wave26 closure card only; MeetEval official cpWER narrow dry-run refresh remains "
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave25_polish_rows()
    if any(row["section_id"] == "frontier_wave26" for row in rows):
        return rows
    return rows + [build_wave26_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave26_presentation_extension_complete",
        "storyboard_scope": "wave26_storyboard_extension",
        "expected_inputs": "Wave26 exploration+baseline closure card.",
        "expected_outputs": "Wave26 storyboard extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave26 storyboard extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave26_presentation_extension_complete",
        "walkthrough_scope": "wave26_walkthrough_extension",
        "expected_inputs": "Wave26 exploration+baseline closure card.",
        "expected_outputs": "Wave26 walkthrough extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave26 walkthrough extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave26_presentation_polish_extension",
        "storyboard_receipt_status": "wave26_presentation_extension_complete",
        "walkthrough_receipt_status": "wave26_presentation_extension_complete",
        "polish_section_count": str(len(polish_rows)),
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": "Extended presentation polish card with Wave26 exploration+baseline closure section.",
    }
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave26_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave26_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave26_presentation_writeback.md"
    ...
    fill_md.write_text(
        "\n".join(
            [
                "# Demo Wave26 Presentation Writeback",
                "",
...
def run_wave26_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave26_receipt = load_json_dict("results/tables/wave26_exploration_baseline_closure_receipt.json")
    wave25_fill = load_json_dict("results/tables/demo_wave25_presentation_writeback.json")
    assert_extension_preconditions(wave26_receipt, wave25_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave26_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave26_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave26 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave26 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave26_presentation_writeback(force=args.force)
