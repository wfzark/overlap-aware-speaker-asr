...
def build_runbook_card_row(
    ...
    if not operator_brief:
        return {}
    ready_handoff = next((row for row in handoff_rows if row.get("action_lane") == "ready_lane"), {})
    target_artifact = str(ready_handoff.get("expected_outputs", ""))
    ...
    return {
        ...
        "runbook_note": (
            f"Start with {frontier} as the current ready lane after confirming the status/handoff bridge checkpoints. "
...
def build_runbook_card_lines(row: dict[str, str]) -> list[str]:
    return [
        ...
        "",
        "This generated runbook card condenses the first top-level status/handoff action into a one-page execution card. "
...
def write_outputs(row: dict[str, str]) -> tuple[Path, Path, Path]:
    ...
    csv_path = tables_dir / "frontier_operator_next_action_status_handoff_runbook_card.csv"
    json_path = tables_dir / "frontier_operator_next_action_status_handoff_runbook_card.json"
    md_path = figures_dir / "frontier_operator_next_action_status_handoff_runbook_card.md"
...
def main() -> None:
    ...
    if not row:
        print("Status handoff operator brief not found; runbook card not written.")
        return
    csv_path, json_path, md_path = write_outputs(row)
    print(
        "Wrote frontier operator next-action status handoff runbook card CSV: "
        f"{csv_path.relative_to(PROJECT_ROOT)}"
    )
    print(
        "Wrote frontier operator next-action status handoff runbook card JSON: "
        f"{json_path.relative_to(PROJECT_ROOT)}"
    )
    print(
        "Wrote frontier operator next-action status handoff runbook card note: "
