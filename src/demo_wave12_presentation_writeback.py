...
from .demo_wave11_presentation_writeback import build_extended_polish_rows as build_wave11_polish_rows
...
def assert_extension_preconditions(
    ...
    wave11_fill: dict[str, Any],
) -> None:
    if str(wave12_receipt.get("execution_status", "")) != "wave12_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave12 exploration baseline closure must be complete before wave12 demo extension"
        )
    if str(wave11_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave11 presentation writeback must be filled before wave12 extension")
    if str(wave11_fill.get("storyboard_receipt_status", "")) != "wave11_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave11 storyboard receipt must be wave11_presentation_extension_complete before wave12 extension"
        )


def build_wave12_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave12",
        "headline": "Wave12 exploration+baseline closure and phase3 gold surface refresh boundary",
        "artifact_anchor": "results/figures/wave12_exploration_baseline_closure_card.md",
        "presentation_note": (
            "Show Wave12 closure card only; external validation and phase3 gate remain "
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave11_polish_rows()
    if any(row["section_id"] == "frontier_wave12" for row in rows):
        return rows
    return rows + [build_wave12_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave12_presentation_extension_complete",
        "storyboard_scope": "wave12_storyboard_extension",
        "expected_inputs": "Wave12 exploration+baseline closure card.",
        "expected_outputs": "Wave12 storyboard extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave12 storyboard extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave12_presentation_extension_complete",
        "walkthrough_scope": "wave12_walkthrough_extension",
        "expected_inputs": "Wave12 exploration+baseline closure card.",
        "expected_outputs": "Wave12 walkthrough extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave12 walkthrough extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave12_presentation_polish_extension",
        "storyboard_receipt_status": "wave12_presentation_extension_complete",
        "walkthrough_receipt_status": "wave12_presentation_extension_complete",
        "polish_section_count": str(len(polish_rows)),
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": "Extended presentation polish card with Wave12 exploration+baseline closure section.",
    }
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave12_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave12_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave12_presentation_writeback.md"
    ...
    fill_md.write_text(
        "\n".join(
            [
                "# Demo Wave12 Presentation Writeback",
                "",
...
def run_wave12_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave12_receipt = load_json_dict("results/tables/wave12_exploration_baseline_closure_receipt.json")
    wave11_fill = load_json_dict("results/tables/demo_wave11_presentation_writeback.json")
    assert_extension_preconditions(wave12_receipt, wave11_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave12_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave12_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave12 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave12 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave12_presentation_writeback(force=args.force)
