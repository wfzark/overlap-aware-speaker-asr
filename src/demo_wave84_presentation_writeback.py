...
from .demo_wave83_presentation_writeback import build_extended_polish_rows as build_wave83_polish_rows
...
def assert_extension_preconditions(
    ...
    wave83_fill: dict[str, Any],
) -> None:
    if str(wave84_receipt.get("execution_status", "")) != "wave84_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave84 exploration baseline closure must be complete before wave84 demo extension"
        )
    if str(wave83_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave83 presentation writeback must be filled before wave84 extension")
    if str(wave83_fill.get("storyboard_receipt_status", "")) != "wave83_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave83 storyboard receipt must be wave83_presentation_extension_complete before wave84 extension"
        )


def build_wave84_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave84",
        "headline": "Wave84 exploration+baseline closure and MeetEval official narrow dry-run speaker-profile diagnostic refresh boundary",
        "artifact_anchor": "results/figures/wave84_exploration_baseline_closure_card.md",
        "presentation_note": (
            "Show Wave81 closure card only; MeetEval official narrow dry-run coordination remains "
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave83_polish_rows()
    if any(row["section_id"] == "frontier_wave84" for row in rows):
        return rows
    return rows + [build_wave84_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave84_presentation_extension_complete",
        "storyboard_scope": "wave84_storyboard_extension",
        "expected_inputs": "Wave84 exploration+baseline closure card.",
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave84_presentation_extension_complete",
        "walkthrough_scope": "wave84_walkthrough_extension",
        "expected_inputs": "Wave84 exploration+baseline closure card.",
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave84_presentation_polish_extension",
        "storyboard_receipt_status": "wave84_presentation_extension_complete",
        "walkthrough_receipt_status": "wave84_presentation_extension_complete",
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave84_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave84_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave84_presentation_writeback.md"
    ...
    fill_md.write_text(
        "\n".join(
            [
                "# Demo Wave84 Presentation Writeback",
                "",
...
def run_wave84_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave84_receipt = load_json_dict("results/tables/wave84_exploration_baseline_closure_receipt.json")
    wave83_fill = load_json_dict("results/tables/demo_wave83_presentation_writeback.json")
    assert_extension_preconditions(wave84_receipt, wave83_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave84_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave84_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave84 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave84 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave84_presentation_writeback(force=args.force)
