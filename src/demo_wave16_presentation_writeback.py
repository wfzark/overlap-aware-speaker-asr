...
from .demo_wave15_presentation_writeback import build_extended_polish_rows as build_wave15_polish_rows
...
def assert_extension_preconditions(
    ...
    wave15_fill: dict[str, Any],
) -> None:
    if str(wave16_receipt.get("execution_status", "")) != "wave16_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave16 exploration baseline closure must be complete before wave16 demo extension"
        )
    if str(wave15_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave15 presentation writeback must be filled before wave16 extension")
    if str(wave15_fill.get("storyboard_receipt_status", "")) != "wave15_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave15 storyboard receipt must be wave15_presentation_extension_complete before wave16 extension"
        )


def build_wave16_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave16",
        "headline": "Wave16 exploration+baseline closure and external validation refresh boundary",
        "artifact_anchor": "results/figures/wave16_exploration_baseline_closure_card.md",
        "presentation_note": (
            "Show Wave16 closure card only; external validation and LLM critic dry-run remain "
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave15_polish_rows()
    if any(row["section_id"] == "frontier_wave16" for row in rows):
        return rows
    return rows + [build_wave16_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave16_presentation_extension_complete",
        "storyboard_scope": "wave16_storyboard_extension",
        "expected_inputs": "Wave16 exploration+baseline closure card.",
        "expected_outputs": "Wave16 storyboard extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave16 storyboard extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave16_presentation_extension_complete",
        "walkthrough_scope": "wave16_walkthrough_extension",
        "expected_inputs": "Wave16 exploration+baseline closure card.",
        "expected_outputs": "Wave16 walkthrough extension under qualitative/demo labeling.",
        "writeback_note": (
            "Wave16 walkthrough extension recorded; no live demo, recording, or benchmark timing claimed."
        ),
    }
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave16_presentation_polish_extension",
        "storyboard_receipt_status": "wave16_presentation_extension_complete",
        "walkthrough_receipt_status": "wave16_presentation_extension_complete",
        "polish_section_count": str(len(polish_rows)),
        "blocker": "controlled_benchmark_timing_pending",
        "fill_note": "Extended presentation polish card with Wave16 exploration+baseline closure section.",
    }
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave16_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave16_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave16_presentation_writeback.md"
    ...
    fill_md.write_text(
        "\n".join(
            [
                "# Demo Wave16 Presentation Writeback",
                "",
...
def run_wave16_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave16_receipt = load_json_dict("results/tables/wave16_exploration_baseline_closure_receipt.json")
    wave15_fill = load_json_dict("results/tables/demo_wave15_presentation_writeback.json")
    assert_extension_preconditions(wave16_receipt, wave15_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave16_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave16_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave16 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave16 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave16_presentation_writeback(force=args.force)
