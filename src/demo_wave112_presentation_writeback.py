...
from .demo_wave111_presentation_writeback import build_extended_polish_rows as build_wave111_polish_rows
...
def assert_extension_preconditions(
    ...
    wave111_fill: dict[str, Any],
) -> None:
    if str(wave112_receipt.get("execution_status", "")) != "wave112_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave112 exploration baseline closure must be complete before wave112 demo extension"
        )
    if str(wave111_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave111 presentation writeback must be filled before wave112 extension")
    if str(wave111_fill.get("storyboard_receipt_status", "")) != "wave111_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave111 storyboard receipt must be wave111_presentation_extension_complete before wave112 extension"
        )


def build_wave112_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave112",
        "headline": "Wave112 exploration+baseline closure after Wave111 HeavyOverlap chain boundary",
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave111_polish_rows()
    if any(row["section_id"] == "frontier_wave112" for row in rows):
        return rows
    return rows + [build_wave112_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave112_presentation_extension_complete",
        "storyboard_scope": "wave112_storyboard_extension",
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave112_presentation_extension_complete",
        "walkthrough_scope": "wave112_walkthrough_extension",
        "expected_inputs": "Wave104 exploration+baseline closure card.",
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave112_presentation_polish_extension",
        "storyboard_receipt_status": "wave112_presentation_extension_complete",
        "walkthrough_receipt_status": "wave112_presentation_extension_complete",
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave112_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave112_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave112_presentation_writeback.md"
...
def run_wave112_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave112_receipt = load_json_dict("results/tables/wave112_exploration_baseline_closure_receipt.json")
    wave111_fill = load_json_dict("results/tables/demo_wave111_presentation_writeback.json")
    assert_extension_preconditions(wave112_receipt, wave111_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave112_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave112_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave112 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave112 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave112_presentation_writeback(force=args.force)
