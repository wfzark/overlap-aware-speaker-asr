...
def build_runbook_card_row(
    ...
    target_artifact = str(current_handoff.get("expected_outputs", ""))
    return {
        ...
        "completion_signal": (
            f"execution queue verification is complete and the target artifact {target_artifact} is ready to open"
            if target_artifact
            else "execution queue verification is complete and the target artifact is ready to open"
        ),
        "urgency": str(operator_brief.get("operator_urgency", "")),
        "runbook_note": (
            f"Start with {frontier} as the current first execution-queue target after confirming the packet and bridge layers. "
...
def build_runbook_card_lines(row: dict[str, str]) -> list[str]:
    return [
        ...
        "",
        "This generated runbook card condenses the first execution-queue action into a one-page execution card. "
...
def write_outputs(row: dict[str, str]) -> tuple[Path, Path, Path]:
    ...
    csv_path = tables_dir / "frontier_execution_queue_runbook_card.csv"
    json_path = tables_dir / "frontier_execution_queue_runbook_card.json"
    md_path = figures_dir / "frontier_execution_queue_runbook_card.md"
...
def main() -> None:
    ...
    if not row:
        print("Execution queue operator brief not found; runbook card not written.")
        return
    csv_path, json_path, md_path = write_outputs(row)
    print(f"Wrote frontier execution queue runbook card CSV: {csv_path.relative_to(PROJECT_ROOT)}")
    print(f"Wrote frontier execution queue runbook card JSON: {json_path.relative_to(PROJECT_ROOT)}")
    print(f"Wrote frontier execution queue runbook card note: {md_path.relative_to(PROJECT_ROOT)}")
