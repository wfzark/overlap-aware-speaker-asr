...
RECEIPT_COLUMNS = [
    ...
    "character_level_coordination_status",
...
def assert_writeback_preconditions(
    ...
    character_coord_receipt: dict[str, Any],
) -> None:
    if str(wave14_receipt.get("execution_status", "")) != "wave14_exploration_baseline_closure_complete":
        raise RuntimeError("Wave14 closure must be complete before MeetEval official narrow dry-run coordination")
    if str(demo_wave14_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave14 presentation writeback must be filled before MeetEval official coordination")
    if str(demo_wave14_fill.get("storyboard_receipt_status", "")) != "wave14_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave14 storyboard receipt must be wave14_presentation_extension_complete before MeetEval official coordination"
        )
    if str(character_coord_receipt.get("execution_status", "")) != "meeteval_cpwer_narrow_dry_run_coordination_complete":
        raise RuntimeError(
            "Character-level MeetEval narrow dry-run coordination must be complete before official coordination"
        )
    ...
    return [
        {
            ...
            "headline": "Official MeetEval cpWER narrow dry-run complete on 5/5 gold cases",
        ...
        {
            "section_id": "character_level_prerequisite",
            "headline": "Character-level narrow dry-run coordination remains the Wave10 prerequisite chain",
            "artifact_anchor": "results/figures/meeteval_cpwer_narrow_dry_run_coordination_card.md",
            "coordination_note": "Official coordination does not replace character-level boundary documentation.",
            "result_label": "experimental/frontier",
        },
        {
            "section_id": "wave14_boundary",
            "headline": "Wave14 closure keeps official MeetEval coordination separate from gold baseline CER",
        ...
        {
            "section_id": "tokenization_reconciliation",
            "headline": "Tokenization diagnostic reconciles official vs character-level cpWER surfaces",
            "artifact_anchor": "results/tables/meeteval_cpwer_official_execution_tokenization_diagnostic.json",
...
def build_fill_row(rows: list[dict[str, str]], official_dry_run_case_count: str) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "meeteval_official_narrow_dry_run_coordination_card",
        "coordination_section_count": str(len(rows)),
        "official_dry_run_case_count": official_dry_run_case_count,
        "execution_receipt_status": "meeteval_official_narrow_dry_run_coordination_complete",
        "blocker": "official_benchmark_claims_still_blocked",
        "fill_note": (
            "Documented MeetEval official narrow dry-run boundary after Wave14 closure; "
...
def build_receipt_row(
    ...
    return {
        ...
        "coordination_scope": "wave14_meeteval_official_narrow_dry_run",
        "wave14_closure_status": str(wave14_receipt.get("execution_status", "")),
        "character_level_coordination_status": str(character_coord_receipt.get("execution_status", "")),
        "official_dry_run_count": official_count,
        "expected_inputs": (
            "Official execution, bridge checklist, character-level coordination, and Wave14 exploration baseline closure."
        ),
        "writeback_note": (
...
def build_card_lines(rows: list[dict[str, str]]) -> list[str]:
    lines = [
        "# MeetEval Official Narrow Dry-Run Coordination Card (experimental/frontier)",
        "",
        "Official narrow dry-run boundary coordination — not an external benchmark completion claim.",
        "",
...
def build_fill_lines(row: dict[str, str]) -> list[str]:
    return [
        "# MeetEval Official Narrow Dry-Run Coordination Writeback",
        "",
...
def build_receipt_lines(row: dict[str, str]) -> list[str]:
    return [
        "# MeetEval Official Narrow Dry-Run Coordination Receipt",
        "",
        "| execution_status | official_dry_run_count | wave14_closure_status |",
        "| --- | ---: | --- |",
        (
            f"| {row['execution_status']} | {row['official_dry_run_count']} | {row['wave14_closure_status']} |"
        ),
    ]
...
def write_outputs(
    ...
    card_csv = tables_dir / "meeteval_official_narrow_dry_run_coordination_card.csv"
    card_json = tables_dir / "meeteval_official_narrow_dry_run_coordination_card.json"
    card_md = figures_dir / "meeteval_official_narrow_dry_run_coordination_card.md"
    fill_csv = tables_dir / "meeteval_official_narrow_dry_run_coordination_writeback.csv"
    fill_json = tables_dir / "meeteval_official_narrow_dry_run_coordination_writeback.json"
    fill_md = figures_dir / "meeteval_official_narrow_dry_run_coordination_writeback.md"
    receipt_json = tables_dir / "meeteval_official_narrow_dry_run_coordination_receipt.json"
    receipt_md = figures_dir / "meeteval_official_narrow_dry_run_coordination_receipt.md"
...
def run_coordination_writeback(force: bool = False) -> dict[str, str]:
    ...
    character_coord_receipt = load_json_dict(
        "results/tables/meeteval_cpwer_narrow_dry_run_coordination_receipt.json"
    )
    assert_writeback_preconditions(wave14_receipt, demo_wave14_fill, character_coord_receipt)

    receipt_path = PROJECT_ROOT / "results/tables/meeteval_official_narrow_dry_run_coordination_receipt.json"
    if receipt_path.exists() and not force:
        existing = load_json_dict("results/tables/meeteval_official_narrow_dry_run_coordination_receipt.json")
        if str(existing.get("execution_status", "")) == "meeteval_official_narrow_dry_run_coordination_complete":
            return {
    ...
    receipt_row = build_receipt_row(wave14_receipt, character_coord_receipt, official_count)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Write MeetEval official narrow dry-run coordination after Wave14 closure."
    )
