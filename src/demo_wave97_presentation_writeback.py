...
from .demo_wave96_presentation_writeback import build_extended_polish_rows as build_wave96_polish_rows
...
def assert_extension_preconditions(
    ...
    wave96_fill: dict[str, Any],
) -> None:
    if str(wave97_receipt.get("execution_status", "")) != "wave97_exploration_baseline_closure_complete":
        raise RuntimeError(
            "Wave97 exploration baseline closure must be complete before wave97 demo extension"
        )
    if str(wave96_fill.get("fill_status", "")) != "writeback_filled":
        raise RuntimeError("Demo Wave96 presentation writeback must be filled before wave97 extension")
    if str(wave96_fill.get("storyboard_receipt_status", "")) != "wave96_presentation_extension_complete":
        raise RuntimeError(
            "Demo Wave96 storyboard receipt must be wave96_presentation_extension_complete before wave97 extension"
        )


def build_wave97_section() -> dict[str, str]:
    return {
        "section_id": "frontier_wave97",
        "headline": "Wave97 exploration+baseline closure after Wave96 MeetEval chain (cycle restart) boundary",
...
def build_extended_polish_rows() -> list[dict[str, str]]:
    rows = build_wave96_polish_rows()
    if any(row["section_id"] == "frontier_wave97" for row in rows):
        return rows
    return rows + [build_wave97_section()]


def build_storyboard_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave97_presentation_extension_complete",
        "storyboard_scope": "wave97_storyboard_extension",
        "expected_inputs": "Wave97 exploration+baseline closure card.",
...
def build_walkthrough_receipt_row() -> dict[str, str]:
    return {
        "execution_status": "wave97_presentation_extension_complete",
        "walkthrough_scope": "wave97_walkthrough_extension",
        "expected_inputs": "Wave97 exploration+baseline closure card.",
...
def build_fill_row(polish_rows: list[dict[str, str]]) -> dict[str, str]:
    return {
        ...
        "writeback_scope": "wave97_presentation_polish_extension",
        "storyboard_receipt_status": "wave97_presentation_extension_complete",
        "walkthrough_receipt_status": "wave97_presentation_extension_complete",
...
def write_outputs(
    ...
    fill_csv = tables_dir / "demo_wave97_presentation_writeback.csv"
    fill_json = tables_dir / "demo_wave97_presentation_writeback.json"
    fill_md = figures_dir / "demo_wave97_presentation_writeback.md"
    ...
    fill_md.write_text(
        "\n".join(
            [
                "# Demo Wave97 Presentation Writeback",
                "",
...
def run_wave97_presentation_writeback(force: bool = False) -> dict[str, str]:
    wave97_receipt = load_json_dict("results/tables/wave97_exploration_baseline_closure_receipt.json")
    wave96_fill = load_json_dict("results/tables/demo_wave96_presentation_writeback.json")
    assert_extension_preconditions(wave97_receipt, wave96_fill)

    fill_path = PROJECT_ROOT / "results/tables/demo_wave97_presentation_writeback.json"
    if fill_path.exists() and not force:
        existing = load_json_dict("results/tables/demo_wave97_presentation_writeback.json")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extend demo presentation polish card with Wave97 section.")
    parser.add_argument("--force", action="store_true", help="Overwrite an already-filled wave97 presentation writeback.")
...
def main() -> None:
    _ = load_config()
    args = parse_args()
    result = run_wave97_presentation_writeback(force=args.force)
