# Plan: Frontier Operator Next-Action Status Bridge Checklist

1. Add a focused failing unit test for the bridge checklist row builder.
2. Implement `src/frontier_operator_next_action_status_bridge_checklist.py` using the existing status JSON only.
3. Emit CSV, JSON, and Markdown outputs under `results/`.
4. Update `.gitignore`, `README.md`, `docs/project_state.md`, and `docs/roadmap.md`.
5. Run focused unit tests around the new bridge and adjacent top-level operator status artifacts.
