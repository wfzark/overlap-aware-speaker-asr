# Plan: Frontier Execution Queue Phase Checkpoint Bridge Checklist

1. Add a focused failing unit test for the phase checkpoint bridge checklist row builder.
2. Extend the execution queue handoff packet coverage so it expects the new bridge section.
3. Implement `src/frontier_execution_queue_phase_checkpoint_bridge_checklist.py` using the existing phase checkpoint card JSON only.
4. Refresh the execution queue handoff packet so it includes the new phase checkpoint bridge section.
