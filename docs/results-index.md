...
| `results/frontier/llm_speaker_attribution/FINDINGS.md` | LLM speaker-attribution repair (ASR×LLM, core speaker axis, #838): the LLM's per-track valence STRONGLY encodes con/pro role (AUC 0.22, strength 0.78) but in a COUNTERINTUITIVE direction — naive reference-free re-attribution scores 0.08, while a sign-calibrated rule would reach 0.92. The reference-free limit is the SIGN, not the signal. (cpCER is permutation-invariant ⇒ swap-blind, so fixed-assignment attribution accuracy is used) | experimental/frontier; Whisper-tiny + deepseek-r1 cached + silver refs; not a gold result |
| `docs/frontier/asr_llm_emotion_capstone.md` | **CAPSTONE** — synthesis of the ASR×LLM + emotion + speaker frontier (#831/#833/#835/#814/#838): the unifying reference-free thread (cheap decoder signal wins routing; acoustic wins emotion; LLM gives coverage not free decision-rules) + a deployable recipe + hero figure (`results/frontier/asr_llm_frontier_capstone.png`) | qualitative/demo synthesis of experimental/frontier; not a gold result |
